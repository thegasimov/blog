...
@section('content')
    <main class="content">
        <div class="container-fluid">
            ...
            <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data" id="createForm" novalidate>
                <div class="row">
                                    ...
                                    @foreach(['az' => 'Azərbaycanca', 'en' => 'İngilizcə', 'ru' => 'Rusca'] as $lang => $label)
                                        <div class="tab-pane fade {{ request()->query('lang', 'az') === $lang ? 'show active' : '' }}" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}-tab">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" value="{{ old('name_'.$lang) }}" placeholder="Bloq adını daxil edin" class="form-control @error('name_'.$lang) is-invalid @enderror" id="name_{{ $lang }}" name="name_{{ $lang }}">
                                                @error('name_'.$lang)
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <textarea name="content_{{ $lang }}" id="content_{{ $lang }}" class="form-control @error('content_'.$lang) is-invalid @enderror">{{ old('content_'.$lang) }}</textarea>
                                                @error('content_'.$lang)
                                                <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="title_{{ $lang }}" id="title_{{ $lang }}" class="form-control @error('title_'.$lang) is-invalid @enderror" value="{{ old('title_'.$lang) }}" />
                                                @error('title_'.$lang)
                                                <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="description_{{ $lang }}" id="description_{{ $lang }}" class="form-control @error('description_'.$lang) is-invalid @enderror" value="{{ old('description_'.$lang) }}" />
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="mb-3" id="published_at_wrapper">
                                    ...
                                    <input type="date" class="form-control" name="published_at" id="published_at" value="{{ date('Y-m-d') }}" />
                                    @error('published_at')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select disabled class="form-control @error('author_id') is-invalid @enderror" id="author_id" name="author_id" >
                                        @foreach($authors as $author)
                                            <option value="{{ $author->id }}" @selected($author->id === auth()->user()->id)>{{ $author->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('author_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
                ...
                ClassicEditor
                    ...
                    .then(editor => {
                        editor.model.document.on('change:data', () => {
                            // Update hidden file input name attributes to avoid the required error
                            const ckHiddenInputs = document.querySelectorAll('.ck-hidden[type="file"]');
                            ckHiddenInputs.forEach(input => {
            ...
            function updateRequiredFields() {
                ...
                ['az', 'en', 'ru'].forEach(lang => {
                    ...
                    document.querySelectorAll(`#${lang} input, #${lang}`).forEach(element => {
                        if (isActive) {
            ...
            function togglePublishedAt() {
                ...
                if (status === 'published') {
                    ...
                    publishedAtField.querySelector('input').setAttribute('required', 'required');
                } else {
                    publishedAtField.style.display = 'none';
                    publishedAtField.querySelector('input').removeAttribute('required');
                }
            }
